...
https://binarysearch.com/problems/Maximum-Stack/editorials/2411774
...
Intuition
Use two sets and keep an increasing id. One is used to store [id, val] pairs, while the other is used to store [val, id] pairs. For pop(), we pop from the first set the pair with maximum id, get its value, and then erase the corresponding pair in the second set. For popmax(), we pop from the second set the pair with maximum value, get its id, and then erase the corresponding pair in the first set.
